...
flags.DEFINE_string(
    "servable_dir", None,
    "The output directory where the servable model will be placed.")
...
def load_labels():
    """loads the labels from the labels.txt file
    TODO use the dynamic data_dir parameter instead of input-data dir"""
    labels = []
    with open('./input-data/labels.txt', 'r') as labels_txt:
        for line in labels_txt.readlines():
            labels += [line.split(",")[0]]
    if not labels:
        sys.exit("No labels found in ./input-data/labels.txt")
    labels.sort(key=int)
    return labels
...
def main(_):
    ...
    if FLAGS.do_serve:
        ...
        path = estimator.export_savedmodel(FLAGS.servable_dir, serving_input_fn)
        print(path)
